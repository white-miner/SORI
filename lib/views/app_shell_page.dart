...
class _DraggableChartWriteFabState extends State<_DraggableChartWriteFab>
  ...
  void _fireTap() {
    ...
    _wave
      ..stop()
      ..forward(from: 0);
    // 파장 시작과 동시에 모달 오픈 (제스처 아레나 없이 즉시)
    widget.onPressed();
  }

  void _onPointerDown(PointerDownEvent e, Size size, Offset pos) {
  ...
  void _onPointerMove(PointerMoveEvent e, Size size) {
    if (e.pointer != _activePointer || _pointerStart == null) return;
    final delta = e.position - _pointerStart!;
    if (!_dragging && delta.distance >= _tapSlop) {
      _dragging = true;
    }
    if (_dragging) {
      setState(() {
        _pos = _clamp((_posAtStart ?? _defaultPos(size)) + delta, size);
      });
    }
  }
        ...
        children: [
          ...
          IgnorePointer(
            child: AnimatedBuilder(
              animation: _wave,
              builder: (context, _) {
                if (_wave.value <= 0) return const SizedBox.shrink();
                return CustomPaint(
                  size: const Size(_width + 56, _height + 56),
          ...
          Listener(
            ...
            onPointerDown: (e) => _onPointerDown(e, size, pos),
                    ...
                    child: Stack(
                      ...
                      children: [
                        ...
                        AnimatedBuilder(
                          animation: _wave,
                          builder: (context, _) {
                            final flash = (1.0 - _wave.value).clamp(0.0, 1.0) *
                                (_wave.isAnimating ? 0.28 : 0.0);
                            return DecoratedBox(
                              decoration: BoxDecoration(
...
class _PulseWavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < 2; i++) {
      final stagger = (progress - i * 0.18).clamp(0.0, 1.0);
      if (stagger <= 0) continue;
      final radius = maxR * Curves.easeOut.transform(stagger);
      final opacity = (1.0 - stagger) * 0.45;
      final paint = Paint()
        ..style = PaintingStyle.stroke
        ..strokeWidth = 3.2 * (1.0 - stagger * 0.55)
      ...
      canvas.drawCircle(center, radius * 0.92, fill);
    }
  }
