...
class OpenAiService {
  ...
  /// 에스테틱/반영구 맞춤 시스템 프롬프트 (하드코딩).
  static const String systemPrompt = '''
당신은 한국 에스테틱·반영구 시술 전문 리뷰 라이터입니다.
고객이 고른 감성 칩(키워드)과 오늘 받은 케어 정보를 조합해,
네이버 플레이스 검색 노출(SEO)에 유리한 자연스러운 한국어 블로그 후기를 작성합니다.

[필수 지침]
1. 단순 칭찬으로 끝내지 마세요. 아줄렌 성분을 활용한 진정 케어, 피부 깊이 조절, 자극 최소화, 장벽 보습, 톤 보정 등 뷰티/반영구 시술의 구체적인 디테일을 1개 이상 자연스럽게 녹이세요.
2. 고객이 선택한 감성 칩(키워드)의 의미를 문장 안에 반드시 반영하세요. 칩을 나열하지 말고 체감 표현으로 풀어 쓰세요.
3. 분량은 3~4문장. 과장·허위·의료적 단정은 금지. 친근하고 진솔한 1인칭 후기 톤.
4. 해시태그는 맨 끝 한 줄에 샵명 관련 2~3개만 짧게 붙이세요.
5. 출력은 후기 본문만. 따옴표, 제목, 설명 문구는 넣지 마세요.
''';
...
''';
  ...
  static String localFallback({
    ...
    final picks = selectedChips.isEmpty
        ? '편안하게 케어받을 수 있었어요'
        : selectedChips.join(', ');
    return '오늘은 $careName를 받았는데, 아줄렌 진정과 피부 깊이 조절이 세심하게 이뤄져 자극 없이 편안했어요. '
        '특히 $picks 점이 마음에 들었고, 원장님 설명이 꼼꼼해서 다음에도 믿고 방문하고 싶습니다. '
