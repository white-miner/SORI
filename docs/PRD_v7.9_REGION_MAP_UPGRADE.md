...
**Status:** Draft · C.1 구현 중/완료 후보 · C.2+·C.S1 **승인 대기** · 전면 구현 금지 해제(단계별만)  
...
| **C.S1** | Quiet Local Canvas · 라이트 래스터 후보 2–3 비교 후 **1 URL 교체** · 마커/시트 대비 | Phase C 배포 | ColorFilter/베이지 overlay · SDK 교체 · 다크모드 |
...
**공통 금지:** Timer · Payment · SQL 파괴 · 고객/Visit 지도 · REST 키 클라 · 가짜 중심 · spiderfy · 우측 상단 3개 이상 · 지도 CSS filter / 전면 반투명 감성 overlay.
...
> Phase C.S1 승인: flutter_map 유지. OSM 기본 대신 Quiet Local Canvas용 **라이트 래스터 1개**로 tile URL 교체(또는 후보 비교 후 1개 잠금). ColorFilter·전면 감성 overlay·위성 기본값 금지. 키/약관/attribution/CORS 확인. 마커는 의미색 최대 2·선택 ring만.
...
| 현재 코드 | `region_nearby_map_section.dart` → OSM `tile.openstreetmap.org` 래스터 |
