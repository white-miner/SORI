-- 043: 샵 소유 고객 일괄 삭제 RPC (RLS DELETE 공백 우회 + 트랜잭션)
-- SECURITY DEFINER로 자식 정리 후 customers DELETE. 성공 id[] 반환.
...
drop policy if exists "mvp_customers_delete" on public.customers;
create policy "mvp_customers_delete"
  on public.customers for delete using (true);

drop policy if exists "mvp_charts_delete" on public.customer_charts;
create policy "mvp_charts_delete"
  on public.customer_charts for delete using (true);

drop policy if exists "mvp_reviews_delete" on public.customer_reviews;
create policy "mvp_reviews_delete"
  on public.customer_reviews for delete using (true);

drop policy if exists "mvp_care_diary_notes_delete" on public.care_diary_notes;
create policy "mvp_care_diary_notes_delete"
  on public.care_diary_notes for delete using (true);
