-- 089_whisper_audience_following_and_labels.sql
-- Whisper composer audience (PO):
--   everyone, followers, following, super_fans, visited(chart customers),
--   peer_directors(=all directors), customer_mode(=regular users), explicit
-- Adds `following` atom (bit 256). Refines visited + peer_directors resolve.
...
comment on function public.resolve_whisper_audience_users(uuid, uuid, text, text[], uuid[], uuid[], int) is
  'Resolve Whisper audience atoms. everyone(32), visited/chart(1), followers(2), following(256), peer_directors/all directors(4), super_fans(8), explicit(16), seminar_hosts(64), customer_mode(128).';
